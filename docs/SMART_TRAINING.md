...
4. Use adaptive practice for suggested targets, or choose up to three keys, 2–4-character sequences (including spaces), or two-word phrases. Phrase targets preserve word order and adjacency, and both words must be in the practice vocabulary. Drills include roughly 70% targeted words and 30% general vocabulary from Monkeytype's English 1k list.
...
Space is part of sequence tracking: `e of` in “bake off” is a four-character target. Leading and trailing spaces also count (` of`, `e `), as do short sequences spanning two boundaries (` a b`). These use the same pair/triple/quad maps and remain enabled when whole-word combinations are switched off. Target labels show spaces as `␣`; manual input accepts either literal spaces or `␣` and preserves boundary spaces. Entries up to four characters are treated as sequences; selecting a word-pair recommendation retains its exact phrase meaning even when its text matches a short sequence. Drills combine matching word endings, complete intervening words, and word beginnings, keeping the transition together within the requested word count.

The **vocabulary** selector switches between the existing English 1k set and **Top 200 English words**. The restricted set is the first 200 lowercase ASCII alphabetic entries in wordfreq 3.1.1's English frequency ranking; numbers, punctuation, and contractions are excluded. The limit applies to every word in diagnostic, adaptive, and manual drills, including words used for space transitions and exact word pairs. The top-200 diagnostic is a fixed 25-word sample (every eighth entry), so it is repeatable within this vocabulary; it does not claim alphabet coverage. Unsupported recommendations are hidden and unsupported selected targets are removed. Switching back restores recommendations from the shared saved profile. The preference is stored in `monkeytype.smartTraining.vocabulary.v1`, changes only between drills, and each new session records its vocabulary for comparison. Older sessions retain their history and display as English 1k.

## Recommendation model

The deterministic engine uses error rate and mean valid transition latency relative to the user's own profile. Errors contribute 70% of the score and slower transitions 30%. Only correct transitions between adjacent characters contribute timing, and intervals outside 30–2000 ms are excluded. Keys require five weighted attempts; two- to four-character sequences require three, each scored against their own sequence-length baseline. Sequence accuracy measures the final-character attempt given a matching prefix; sequence latency measures the final transition, not the duration of the whole sequence. Targets with no measured weakness are omitted.

That weakness score is multiplied by a general-English frequency boost between 1× and 2×. For each sequence, exposure is the sum of word frequency times its number of overlapping occurrences in the word, expressed per million words. The multiplier is `1 + log(1 + exposure) / log(1 + maximum exposure for this sequence length)`, capped at 2. Keys, pairs, triples, and quads therefore use separate frequency scales. Missing sequences receive 1×. Space-containing sequences also receive 1× because the bundled single-word frequency data does not measure cross-word transitions. A common sequence with no measured weakness remains unranked.

The bundled table is derived from the 50,000 highest-ranked English entries in wordfreq 3.1.1, retaining lowercase ASCII alphabetic words. It uses the full selected corpus to calculate exposure and maxima, then ships only sequences supported by both training vocabularies and the diagnostic. No frequency lookup or corpus work runs during typing. See [data sources and attribution](TRAINING_FREQUENCY_DATA.md). Frequency changes recommendations immediately for existing profiles; recorded attempts, errors, timing, accuracy, and history are unchanged. This estimates the everyday usefulness of combinations across words; it does not track weakness in individual whole words.

The **word combinations** switch turns word-pair tracking, recommendations, and manual/adaptive phrase targets on or off between drills. It defaults to on and remembers your choice in this browser. Switching off removes selected phrase targets while keeping letter and space-sequence targets. Disabled drills leave saved word-pair evidence unchanged, including its recency weight, so switching back on restores your recommendations.
